...
function parseInline(text) {
  return text
    ...
    .replace(/---/g, '<hr>') // hr
    .replace(/<a>(.*?)<\/a>/g, '<a href="$1">$1</a>') // link
    .replace(/<blue>(.*?)<\/blue>/g, '<span style="color: rgba(112, 165, 240, 1);;">$1</span>')
    .replace(/<red>(.*?)<\/red>/g, '<span style="color: rgb(240, 112, 112);">$1</span>');
}